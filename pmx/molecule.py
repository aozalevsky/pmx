...
class Molecule(Atomselection):
    ...
    def set_psi_down(self, degree, propagate=True):
        ...
        CA, C = self.fetchm(['CA', 'C'])
        R = Rotation(C.x, CA.x)
        rot_atoms = self.fetch_atoms(
            ['O', 'OXT', 'O1', 'O2', 'OC1', 'OC2', 'CA', 'C'], inv=True)
        if propagate:
            chidx = self.chain.residues.index(self)
            if chidx != 0:
...
class Mol2Molecule:
    ...
    def read(self, lines):
        ...
        self.__parse_bonds()
        if self.keys:
            self.footer = [self.keys[0]] + readSection(
                lines, self.keys[0], 'XX')
    ...
    def write(self, fp=sys.stdout):
        ...
        print >>fp, self.name
        print >>fp, self.num_atoms, self.num_bonds, \
            self.num_substr, self.num_feat, self.num_sets
        ...
        for atom in self.atoms:
            print >>fp, "%7d %-8s %9.4f %9.4f %9.4f %-6s %3d %-8s %9.4f" %\
                (atom.id, atom.symbol, atom.x[
                 0], atom.x[1], atom.x[2], atom.atype,
